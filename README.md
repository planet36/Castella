...
The round constants are the successive states of a 128-bit [Galois LFSR](https://en.wikipedia.org/wiki/Linear-feedback_shift_register#Galois_LFSRs) with the [GCM](https://en.wikipedia.org/wiki/Galois/Counter_Mode) reduction polynomial (x¹²⁸ + x⁷ + x² + x + 1), stepped 128 times between constants.  The LFSR seed, which is also the first round constant, is <q>expand 16-byte c</q>.
...
The round constants are generated at compile time.  Their number is `Castella::NUM_ROUNDS_MAX × Castella::AES_NUM_ROUNDS × Castella::B_MAX`, which is 16 × 3 × 16 = 768.  All three values are fixed by [SPEC.md](SPEC.md), and any other value produces different digests.
...
Round counts appear nowhere in that table, because as in SHA-3 they are safety margin rather than a security parameter.  [SPEC.md](SPEC.md#security-claims-and-non-claims) derives these strengths from the claim, states which `(C, num_rounds)` instances the claim covers, and gives the margin rationale behind the round counts.
...
The [`castella` command-line program](hash-programs/castella.cpp) wraps `DuplexTree` and adds a keyed **MAC mode** (`--key-file`, KMAC's structure at tree scale).  The [`examples/`](examples/) programs show the higher-level SP 800-185 constructions built directly on a single `Duplex`, which are cSHAKE-, KMAC-, TupleHash-, and ParallelHash-like functions.
...
* **Register-resident permutation.**  The permutation (every supported state size _N_) runs in a folded representation that stays in _N_/2 ymm registers for all rounds, where element _j_ holds blocks _j_ and _j_+_N_/2, one per 128-bit lane.  The generic path instead bounces the state through memory between the AES rounds' 256-bit accesses and the transpose's 128-bit accesses, a pattern that defeats store-to-load forwarding.  For the 16-block state used by `Duplex` the folded path measured ~1.7× faster, and a plain duplex absorbs at ~3.2 GiB/s per core instead of ~1.9 before folding.  [research/permute\_folded-benchmark.cpp](research/permute_folded-benchmark.cpp) measured the speedup, with results in [research/README.md](research/README.md), and the ~3.2 absorb rate comes directly from [research/duplex-throughput-benchmark.cpp](research/duplex-throughput-benchmark.cpp).
* **Leaf batching.**  Both tree hashes process adjacent leaf chunks **two at a time on one thread**.  `DuplexTree` packs two duplex states into the two 128-bit lanes of ymm registers ([`Castella::DuplexX2`](include/castella-duplex-x2.hpp)), where VAES applies an independent AES round per lane and the AVX2 unpack network transposes both 16×16 byte matrices at once without mixing the lanes ([`Castella::permute_x2`](include/castella-permute.hpp)).  One paired permutation measures ~1.7× faster than two sequential (register-resident) permutations ([research/permute\_x2-benchmark.cpp](research/permute_x2-benchmark.cpp)).  The `cch` tree instead interleaves two nodes' compression chains in one loop ([`compress_castella_hash_x2`](include/cch-x2.hpp)).  A single cch node is latency-bound, so the second state's chains fill the idle AES slots, for a modest ~1.1× on pinned runs.  [research/simd\_compress-num\_states-benchmark.cpp](research/simd_compress-num_states-benchmark.cpp) measured it, and [research/README.md](research/README.md) has the findings.  Pairing applies on every parallel path: batch workers, the inline (single-threaded) path, and the streaming pipeline.  The pipeline's pool workers claim up to two adjacent ring slots per wake-up, so streamed `castella --no-mmap` input at 2 threads reaches the producer-bound floor that previously needed 4.

All of these ratios are machine-dependent.  To reproduce them, build `research/` (see [Building](#building)) and run `bash run-benchmarks.bash` there.  It pins each benchmark to core 0 and saves the raw results to `research/results/`.  Benchmark on an otherwise idle machine.
...
* `make test` — build and run every test suite, by delegating to each subdirectory's own `test` target.  The two Python steps need `python3`, and say so rather than failing obscurely if it is missing
...
Castella is licensed under the [Mozilla Public License 2.0](https://www.mozilla.org/en-US/MPL/2.0/) (`MPL-2.0`), and every source file carries an SPDX header.
