...
| 2 | 6 | **45** (solved) | 2<sup>−270</sup> | **2<sup>−298</sup>** | [270, 298] |
...
Every round count with both ends now has a **solved** floor, so the remaining width is entirely the trail search's: 163 bits at _r_ = 4, 117 at _r_ = 3, 28 at _r_ = 2.  The _r_ = 4 figure used to read 325 against a superadditive floor of 828 — solving _A_(4) = 165 raised that floor to 990 and halved the apparent gap, which is what a loose floor costs when it is mistaken for the trail's slack.  Every floor is far past 2<sup>−256</sup>.
...
At _r_ = 2 the shell is what the totalizer rescued, and it is the one place it earned its keep.  Under the `pb` weight bound, _K_ = 0, 4 and 8 each returned **zero** trails in 900 s — failing to find even the weight-302 trail that provably satisfies the constraints, so the bound was making the instance harder rather than easier.  With `--weight-encoding totalizer` both _K_ = 0 and _K_ = 4 return one, and the _K_ = 0 trail is the weight-**298** characteristic that moved the ceiling.  That is still not a clustering figure: each run finds its one trail and then times out on the next `check()`, so the shells are reported `INCOMPLETE` and no sum over them means anything.  **Clustering above _r_ = 1 therefore remains the one adverse-direction gap in this evidence** — the _r_ = 1 measurement says the gain is about 2 bits there, and nothing measures how it grows.
...
# The recorded r = 2 ceiling of 298 is NOT what the search prints (302) -- it comes
# from the cluster shell on that trail's own differential.  ~15 min, 308 MB.
python3 permute-trail-search.py -r 2 --patterns 1 -t 900 -M 1500 --no-minimize \
    --weight-encoding totalizer --cluster 500 --cluster-shell 0
