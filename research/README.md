...
Every round count with both ends now has a **solved** floor, so the remaining width is entirely the trail search's: 229 bits at _r_ = 5, 161 at _r_ = 4, 67 at _r_ = 3, 24 at _r_ = 2.  The _r_ = 4 figure used to read 325 against a superadditive floor of 828 — solving _A_(4) = 165 raised that floor to 990 and halved the apparent gap, which is what a loose floor costs when it is mistaken for the trail's slack.  Every floor is far past 2<sup>−256</sup>.
...
**_r_ ≤ 5 is bracketed by solved floors; above that there is a floor and no ceiling.**  The _r_ = 3, _r_ = 4 and _r_ = 5 brackets ([774, 841], [990, 1151] and [1404, 1633]) all rest on solved _A_; every ceiling is a found trail rather than a minimum — the best of 416 trails at _r_ = 3 and of 35 at _r_ = 4, but still not a proven minimum, since the minimization loop has never completed above _r_ = 1 — so no upper end is tight.  (Refutations *have* completed above _r_ = 1 since 2026-08-03, but only with an input/output differential pinned, which bounds that differential rather than the pattern, so no ceiling rests on one.)  At _r_ = 5 and _r_ = 6 the floors are solved too (2<sup>−1404</sup> and 2<sup>−1620</sup>).  _r_ = 5 now has a ceiling as well, but only because the activity pattern is **imported from the MILP** (`--dump-pattern` / `--pattern-file`): stage A still never produces one itself, at any of the six targets tried, under either cardinality encoding, or at any of eight random seeds.  _r_ = 6 has not been given to the trail search at all, and would need the same import to be.  _r_ = 7 and _r_ = 8 have solved floors too (2<sup>−2124</sup> and 2<sup>−2340</sup>), so **every floor from _r_ = 1 to _r_ = 8 is a converged optimum** — that range covers every shipped round count.  The other _r_ = 5 ceilings — [606, 705] at _N_ = 2 and [684, 791] at _N_ = 4 — bound **narrower permutations, not the shipped one**, and must not be read as bounds on _N_ = 16; [1404, 1633] is the one that bounds the shipped width.  _r_ = 7 and _r_ = 8 have solved MILP floors but have never been given to the trail search; _r_ ≥ 9 is unsearched by either tool.  Read these as bounds, not as the permutation's actual trail weights.  See [VERIFYING-CLAIMS.md](VERIFYING-CLAIMS.md) for how these results feed the claim.
...
# lightest.  r = 5's 1633 is a seed sweep only -- --patterns has nothing to enumerate
# when the single pattern comes from a file.
