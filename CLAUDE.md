...
- **`tests/`** — The test programs.
  ...
  - `kat.cpp` + `KAT.txt`: machine-readable KAT file (duplex/tree/mac/cchtree lines, message pattern `msg[i] = i mod 256`, MAC key pattern `key[i] = 255 - (i mod 256)`; the vector count is pinned by `EXPECTED_KATS`, so a truncated file fails rather than passing short); `./kat` verifies it, `./kat --generate > KAT.txt` regenerates (only on a deliberate format change).
