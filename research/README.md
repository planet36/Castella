...
* `-t` is the per-solver-call time limit.  The _r_ = 2 minimization did not finish within 60 min on this machine; the reported 313 is the best trail found across runs, not a proven minimum, and a longer limit may or may not tighten it.  Expect run-to-run variation in which trail is found (313/314/315).
