...
`permute-invariant-subspaces.py` (~14 s, standard library only) replaces the sampling above with exhaustive computation wherever the structure permits.  It imports `permute-trail-search.py` for the cross-validated layer machinery and `spec-conformance.py` for the round function and constant schedule, so no layer is modelled a second time.  All decided checks passed.
