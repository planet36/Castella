...
| 2 | 6 | **45** (solved) | 2<sup>−270</sup> | **2<sup>−294</sup>** | [270, 294] |
...
At _r_ = 2 the shell is what the totalizer rescued, and it is the one place it earned its keep.  Under the `pb` weight bound, _K_ = 0, 4 and 8 each returned **zero** trails in 900 s — failing to find even the weight-302 trail that provably satisfies the constraints, so the bound was making the instance harder rather than easier.  With `--weight-encoding totalizer` the shells return trails, and bisecting the cap downward is what took the ceiling to 294 (see the _r_ = 2 bullet).

**It still does not give a clustering figure, and the reason is sharper than a timeout.**  Every shell at _r_ = 2 that *contains* anything reports `INCOMPLETE`: the run finds its one trail and then times out on the next `check()`, at 900 s and again at 3 600 s on the same instance.  The shells that complete are exactly the **empty** ones — capping the weight below the differential's minimum is refuted in about two minutes, four times over.  So proving a shell empty is easy and exhausting a non-empty one is not, which is the opposite of what the enumeration needs: a DP sum requires every term, and not one non-empty shell has ever been closed above _r_ = 1.  **Clustering above _r_ = 1 therefore remains the one adverse-direction gap in this evidence** — the _r_ = 1 measurement says the gain is about 2 bits there, and nothing measures how it grows.
...
# The recorded r = 2 ceiling of 294 is NOT what the search prints (302) -- it comes
# from bisecting a cluster shell on that trail's own differential.  ~3 min, 320 MB.
# --cluster 1 stops at the first trail; without it a satisfied probe burns a whole
# -t on a second check().  Lower the shell to descend, raise it to refute.
python3 permute-trail-search.py -r 2 --patterns 1 -t 900 -M 1500 --no-minimize \
    --weight-encoding totalizer --cluster 1 --cluster-shell -8
