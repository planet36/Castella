...
# The recorded r = 2 ceiling of 293 is NOT what the search prints (302) -- it comes
# from working a cluster shell on that trail's own differential.  ~4 min, 320 MB.
# --cluster 1 stops at the first trail; without it a satisfied probe burns a whole
# -t on a second check().  Lower the shell to descend, raise it to refute: -10
# (cap 292) times out, and -8 (cap 294) is where the bisection alone stopped.
...
# Enumerating MORE than one trail above r = 1 needs --fresh-instances: this
# returns all 8 in ~491 s, where the same command without the flag returns 1 and
# then reports unknown at 905 s.  The flag only pays across trails, so it is
# pointless on the single-trail descent probe above.
python3 permute-trail-search.py -r 2 --patterns 1 -t 900 -M 4000 --no-minimize \
    --weight-encoding totalizer --cluster 8 --cluster-shell -6 \
    --cluster-time-limit 1200 --fresh-instances

# Soundness gate for that flag, at the one round count where the enumeration
# closes: this must return the SAME 1048 trails as the r = 1 command above, and
# report complete.  ~2 h against that run's 14 s -- the flag costs 530x where
# the incremental solver already works.
python3 permute-trail-search.py -r 1 --patterns 1 -t 600 --encoding rows \
    --cluster 5000 --fresh-instances --cluster-time-limit 12000

# These sweeps give the best TRAIL at r = 3 and r = 4 (841 and 1151); the recorded
# CEILINGS (823 and 1123) then come from a shell descent over that trail followed by
# an enumeration of the shell it stopped in -- see the --cluster-shell block below,
# which is what every ceiling from r = 3 to r = 8 now rests on.  Sweep BOTH the pattern count and the seed: run each K as its own
# process, in parallel, and take the lightest.  The r >= 5 sweeps are seed-only --
# --patterns has nothing to enumerate when the single pattern comes from a file.
# Budget by elapsed time, not by the solve times printed per pattern -- those cover
# check() alone, while the untimed per-pattern model build is the larger share
# (~100 s per pattern at r = 3 against a printed ~8 s).  Under 2 GiB total with -M.
# The r = 4 ceiling came from seeds 0..7 at --patterns 16 (~40 min wall, five trails
# tied at 1151).  At r = 3 that shape gave 846 (seed 0, pattern 12) in ~35 min; the
# wider sweep below -- nine seeds at --patterns 32, ~5.5 h wall -- gives the 841 at
# seed 11, pattern 13.  Use FRESH seeds when raising --patterns: stage A enumerates
# from pattern 1 every run, so re-running a swept seed repeats what it already did.
for K in 8 9 10 11 12 13 14 15 16; do
    python3 permute-trail-search.py -r 3 -A 129 --patterns 32 --random-seed "$K" \
        --no-minimize -t 600 -M 900 > "r3-seed$K.txt" &
done; wait
grep -h 'best characteristic' r3-seed*.txt   # then -r 4 -A 165 --patterns 16 -M 1200
...
# above its own solve time -- 1585 s, 7257 s and 14050 s -- since the 600 s
# default silently cuts all three off.  Stage B is never the obstacle there
# (7.6 s, 11 s, 14 s); the untimed model build is ~90% of each trail-search run,
# so budget ~4-5 min per seed.  Solve times are single-core: HiGHS does not
# parallelize this model, so the three MILPs can run concurrently at no cost.
# Winning seeds were 2 (1887), 8 (2473) and 1 (2725); the sweeps spanned only
# 1887-1890, 2473-2478 and 2725-2729, so seeds are worth 1-3 bits at most here.
# That is a statement about SEEDS, not about the ceilings: a shell descent over
# seed 2's, 8's and 1's trails then took r = 6 to 1857, r = 7 to 2448 and r = 8
# to 2705 -- 30, 25 and 20 bits that no further seed was going to find -- and
...
# lighter ones inside the SAME pattern and the SAME input/output difference.  A
# --cluster 1 probe is the right shape; without it a satisfied probe spends a whole
# -t on a second check() nobody asked for.  --weight-encoding totalizer is required:
# under the pb default the shell returns nothing.  Run several K in parallel and take
# the lowest that comes back sat -- each is an independent ceiling.
#   r = 3: best 841 -> 840, 838, 834, 829, 827, 825, 824 as K went -1 .. -17
python3 permute-trail-search.py -r 3 -A 129 --patterns 13 --random-seed 11 \
...
# THE SHELL ENUMERATION -- where each ceiling above actually ends up.  Once the
# descent stalls, do NOT keep probing below its last satisfied cap: re-ask THAT cap
# for many trails.  A --cluster 1 probe takes z3's first answer and that is the
# heaviest the bound allows, so the lighter characteristics in the same shell are
# reachable only by asking for more of them.  --fresh-instances is required above
# r = 1; without it the run returns one trail and stalls on the next check().
...
# Give r = 5 -M 4000, not 2500: at 2500 it ends `unknown: max. memory exceeded`
# after ONE trail at 8628 s of its 14400 s, so that one is memory-bound rather
# than time-bound.  All six ran INCOMPLETE, which costs a CEILING nothing -- any
# trail below the cap is the result -- but leaves their DP sums meaningless.
# Three of the six (r = 3, 5, 8) returned their best trail LAST, which looked
# budget-limited -- but re-running all six on 2026-08-08, three of them at
# double the budget, moved NO ceiling.  A longer -t is not the lever it looked
# like; at 14400 the runs above reproduce trail for trail.
# A negative shell that returns UNSAT is a COMPLETE result for that differential:
# caps 791 (r=3), 990 (r=4), 1404 (r=5), 1620 (r=6), 2124 (r=7) and 2340 (r=8) each
# exhausted their shell, proving those differentials admit nothing lighter.  Every
# round count from 3 to 8 now has one.  Note how weak the r >= 4 ones are, though:
# their caps ARE the idealized floor 6*A, so they prove only that the differential
# cannot ATTAIN the floor, leaving everything from 6*A+1 up to the boundary untested.
# Only r = 3's sits above its floor (791 against 774), ruling out a 17-bit band.
# A timeout is `unknown` and bounds NOTHING in
# either direction -- and several here are budget artifacts rather than walls, since
# r = 3's cap 824 timed out at 3600 s and then solved in 1387 s when given 14400 s,
# and its cap 823 timed out TWICE, once at the full 14400 s, before the enumeration
# above produced a weight-823 trail.  An `unknown` was never evidence of absence.
# WHERE TO RESUME.  Each ceiling's boundary -- lightest realized weight, tightest cap
# that timed out -- is (821, 823] at r=3, (1115, 1123] at r=4, (1593, 1602] at r=5,
# (1842, 1856] at r=6, (2423, 2447] at r=7 and (2685, 2699] at r=8.  Every round
# count now has a failed cap below its ceiling, so none of the six is untested on
# the low side -- but all six failures are timeouts, which bound nothing.  At r = 3,
# 822 has never been asked at all.
# Memory: -M is PER PROCESS, and these probes grow to ~2 GB over a few hours, so a
# parallel batch needs N * M within RAM (8 at -M 2500 left 1.8 GiB of 15 GiB free).

# At narrower N stage A works and finishes in ~2 min, bracketing those
# permutations.  Both use proven A, so both floors are real.
