...
own patterns there in under a second, so `-A 129` and `-A 165` are all that is
...
```bash
# PuLP is not in the system Python here, so this uses the venv.  HiGHS runs
# single-core despite --threads, so these can run concurrently at no cost.
/home/sdw/.venvs/pulp/bin/python permute-min-active-sboxes.py \
    --min-rounds 6 -r 6 -t 21600 --dump-pattern patterns/pat-r6.json
```

Give `-t` room above the solve times in the table, because the 600 s default
silently cuts off every one of these cells, and a timed-out incumbent is not an
optimum.  The `proven_optimal` field in each file records which it is, and only
`true` licenses 6·_A_ as a floor.
...
minimal pattern at these depths, and the ceilings resting on them are upper
bounds of unknown tightness.  See the Scope note in [../README.md](../README.md).
...
instantiating it.  That check would catch a disagreement between the two, and
it costs about 0.5 s, against the 900 s stage A spends failing to find a pattern
it can verify that fast.
