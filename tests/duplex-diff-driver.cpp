...
* The whole run is batched through one process because the Python model is
* the slow side; per-program process startup would be pure overhead.
*
...
* null, and the \c add_left_encoded and \c add_right_encoded span overloads
* treat a null data pointer as a no-op — a C++-side API convenience with no
* counterpart in the specification or the Python model, which would otherwise
* show up as a spurious divergence on every empty-payload program.
*
