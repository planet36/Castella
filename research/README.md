...
* **Inside-out: a zero-sum over 3 rounds, the longest reach demonstrated here.**  From a 2^128 cube filling one block of a middle state, `P` forward covers 2 rounds and `P⁻¹` backward covers 1, so the zero-sum spans `r_fwd + r_bwd` = 3 rounds.  Both halves hold by the counting argument in step 3 above, which is indifferent to direction because it needs only that a round is a bijection on the cube block, so the transpose hands every block one active byte of even multiplicity.
  ...
  **An earlier revision of this section claimed 4 rounds, and that was wrong.**  It came from `--inside-out 2 2 -c block`, whose two halves both report balance — but not for the same cube.  `build_trail` drops the *trailing* transpose going forward, which is free because it only relabels output bits, and the *leading* one going backward, which is not: it relabels the cube.  So the same bit-set names a **row** of the byte matrix to the forward half and a **column** to the backward half, and no middle state is both — they share 1 byte of 16.  A zero-sum is a statement about one set of texts, so the two halves cannot be added.  Measured per cube by brute force (`permute-multiplicity-verify.py`), identically at reduced widths _N_ = 2 and _N_ = 3: a row reaches forward 2 / backward 1, a column forward 1 / backward 2, and a diagonal 1 / 1.  Either way the reach from a single cube is **3**.
  ...
  **The model cannot certify the surviving 3-round construction, and this is a real limit of the technique rather than a budget problem.**  Its backward half is `P⁻¹` for one round from a row cube; the sparse pruning then keeps a single live block, which restricts the cube to the one byte that reaches the target, and over 2^8 the sum is a much harder thing to prove than over 2^128 — the model reports *not provably balanced* in 47 s.  The pruning stays sound (it can only fail to prove balance, never assert it falsely), but the dimensions it discards are exactly the ones supplying the even multiplicity.  So the 3-round result rests on the verified counting argument and the brute force, not on this solver.
