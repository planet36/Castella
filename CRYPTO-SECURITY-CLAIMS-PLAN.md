...
forgery), **not** software security.  Implementation attacks, memory safety, side
channels, and parsers are covered by [ADVERSARIAL-REVIEW-PLAN.md](ADVERSARIAL-REVIEW-PLAN.md)
...
1. **A security claim** (conjecture, stated as a falsifiable contract).  Keccak's is the
   *flat sponge claim*: "no attack succeeds with less workload than the generic attacks
   ...
   cryptanalysts, and it is deliberately *not* a proof.
2. **Proven mode reductions** (theorems, conditional on the claim).  Sponge/duplex
   ...
   that the **mode** adds no weakness, because they reduce every construction's security
   to the permutation's.  These are the only parts that can be *established* in the
   mathematical sense.
3. **Evidence** (accumulated, never finished).  Bounds on differential/linear trails,
   ...
   and, most importantly, the *absence* of attacks over time.  Evidence supports the claim
   but never proves it.
...
In sum, there is **one primitive claim** (on `P`, expressed through the sponge capacity) and
there are **three short mode arguments** (duplex/sponge, tree, and MAC).  Everything else is
either equivalence (already tested) or explicitly out of scope.
...
**SHIPPED.  SPEC.md's "The security claim" is the authoritative wording, and the draft below
is what it was written from, kept for the notes under it.**  Three differences are worth
...
* **Flat, not hermetic.**  A flat claim is made about the *sponge instance*, not about `P`
  ...
  trivial distinguishers (it is efficiently invertible, and it has some fixed structure),
  and the claim is that none of them translate into an attack on the sponge better than
  generic.
* **The claim names its parameters.**  It covers specific `(C, R*)` pairs (§4.3), the
  ...
  *not* covered, because they exist to be attacked (§5.7).
* **Breaking the claim is a result, not a catastrophe.**  Following Keccak, if an attack
  beats the claimed bound, the claim was false and the parameters get revised.  Say this in
  the spec, because it is what makes the claim credible rather than boastful.

### 2.2 Where it lives

Rewrite SPEC.md's "Security claims and non-claims" into three clearly-labeled layers
(§0): **Claim** (the flat sponge claim and the table in §3), **Proven mode reductions**
(sponge/duplex generic bounds, tree soundness, and MAC framing, each with its short argument
or a pointer to it), and **Evidence** (the MILP/diffusion results, with their stated
limits).  The existing candid non-claims stay verbatim.

---

## 3. Security strengths (the calculable part)

These follow *generically* from the claim, so this table is arithmetic, not analysis.  For
an `n`-byte digest with capacity `C` blocks, MAC key `K`, and MAC tag length `L` bytes (all
values in bits):
...
The deliverables are **all shipped**:

* this table in SPEC.md ("Security strengths (generic bounds)")
* the derivation of each row from the flat claim, citing Bertoni et al., *Cryptographic
  sponge functions*, for the generic bounds
* the MAC rows' reduction, which grew from the sketch this section asked for into the full
  argument in SPEC.md's "The keyed construction is a MAC"
...
SHA3-`d` is Keccak with capacity `c = 2d` bits, and its security levels (collision `d/2`,
preimage `d`, 2nd preimage `d`) come **entirely from `c` and `d`**.  The round count (24)
appears nowhere in the security claim, because it is safety margin backing the claim.  The
...
produces exactly this mapping, which is worth stating in the README.

### 4.3 The round count `R*` (methodology, not a formula)

No round count can be *calculated* to "equal" SHA-3, because SHA-3's own margin is
conjectural (24 rounds vs. best practical collision attacks reaching ~6 rounds of the
permutation).  The honest procedure has four steps:
...
1. **Floor from proven trail bounds.**  From the MILP table (research/README.md, `N=16`,
   ...
   r = 8**, which covers every shipped round count: A = 9, 45, 129, 165, 234, 270, 354, and
   390 at r = 1…8.  Above that, superadditivity (`A(a+b) ≥ A(a)+A(b)`, valid because `P` is
   a bijection) composes solved values with each other.  So 6·A ≥ 2b is reached at **r=2**
   for b ≤ 135, **r=3** for b ≤ 387, **r=4** for b ≤ 495, **r=5** for b ≤ 702, **r=6** for
   b ≤ 810, **r=7** for b ≤ 1062, and **r=8** for b ≤ 1170.  That puts the 128-, 256-, and
   512-bit floors at r = 2, 3, and 5 respectively.
   For linear trails, correlation ≤ 2^−3·A gives the same round floors.  These are
   necessary conditions only (single characteristics, with no clustering or structural
   coverage).

   The 512-bit floor of r=5 is now settled from both sides and cannot move.  Four rounds
   support exactly 495 bits, 17 short of 512, and `A(4) = 165` is a converged optimum, not
   a bound, so no further solving can revisit it.

   *Superseded 2026-08-02, twice in one day.*  This step first read r=3 for b ≤ 399 (A=133)
   and r=4 for b ≤ 675 (A=225), both timed-out incumbents mislabeled as optima.  It was then
   corrected to superadditive floors of 138/174 at r = 4/5, which were sound but loose by
   20% and 34%.  The solved values above replace them.

   The 512-bit floor itself moved once.  Under A(4) = 225 it sat at **r=4**
   (6·225 = 1350 ≥ 1024), and refuting that figure is what pushed it to r=5 and created the
   `C` = 8 margin exception in step 4.  The additive policy adopted in step 3 has since
   dissolved that exception, with no round count changing.  Solving A(4) = 165 has now
   fixed the floor there permanently.
2. **Floor from diffusion.**  Full bit diffusion needs r=3 (empirical, corroborated by
   avalanche statistics).
3. **Margin against the unknown.  RESOLVED 2026-08-08, and the adopted policy is
   ...
   nothing shipped changed when it was adopted.  It is written up in SPEC.md's margin
   rationale, including the disclosures below.  Re-evaluate as §5 produces results: a
   distinguisher at 4 rounds obliges `floor + 4` = 7/7/7/9.
   ...
   *Why additive rather than the ratio policy this step used to propose* (`R* ≥ 2× the
   ...
   anchoring on the trail floor, as step 4 did, gives more protection to the capacity whose
   bound is weaker, which is backwards.  Attacks advance round by round.  Note the old
   example policy in this step was never the problem: at a known reach of 3 it demands
   `R* ≥ 6`, which every shipped row already met.  The `C=8` exception came from step 4
   applying `2×` to the *floor* instead.

   *The comparison with Keccak that this step used to make is not like-for-like.*  Keccak's
   ~3–4× is against practical attacks reaching ~6 rounds, while the 3 here is a
   2^128-data distinguisher on `P`, not a break of the sponge.  It cuts both ways, and
   SPEC.md now says so.  There are no practical attacks on Castella at any round count to
   measure from, but zero-sums reach full-round Keccak-`f`, so Keccak would score zero
   margin under the rule adopted here.  Do not present the 2.00×/2.67× ratios as comparable
   to a SHA-3 figure.
4. **Publish per-C recommended `(C, R*)` pairs** in SPEC.md as *the claimed instances*.
   This is **done, and the margin question that hung over this step is CLOSED (2026-08-08).
   The round counts stay at 6/6/6/8 and the rationale was rewritten** under step 3's adopted
   policy `R* = binding floor + 3`.  `C=8` is no longer an exception to anything, and at
   2.67× the known distinguisher reach it is the *widest* margin of the four rows.
   ...
   *How it was decided, since the reasoning is easy to lose.*  The prior framing here was
   ...
   the second-*strongest* row and `C`=6 the weakest).  Margins **in rounds** are 2.00× /
   2.00× / 2.00× / **1.60×** (so `C`=8 is the only row below policy).  Those figures are
   still correct, and every `L(R*)` in them is a solved optimum.

   **Adopting the bits reading was rejected anyway**, on two grounds.  It is the metric
   that happens to flatter the row under question, which a reader is entitled to be
   suspicious of.  And it is the wrong metric for the purpose, since doubling a
   *differential* bound buys nothing against the rebound, integral, and algebraic attacks
   the margin exists to cover.  The additive rounds rule keeps the conservative metric and
   needs no exception, which is why it won.

   **Disclose in SPEC.md, and it does:**

   * that the `+ 3` rule was recognized *after* the round counts were chosen
   * that it replaced a `2 ×` floor rule, and why that rule was defective
   * that the anchor is a *known* reach with no upper bound claimed on the true one
   * the resulting revision trigger, `floor + 4` = 7/7/7/9 if a 4-round distinguisher
     appears

   **The thinnest margin in this table is not `C` = 8 but `C` = 6's floor.**  That row
   needs `A(3) ≥ 128` and gets **129**, so it clears by one S-box, 6 bits out of the 768
   the level requires.  `C` = 6 is the only row where `2b/6` is an integer exactly, which
   is why, since the other three round up and clear by 2, 43, and 63 boxes.  One box lower
   still, at `A(3)` = 127, this row's trail floor would be r=4 and its `R*` 7 rather than
   the shipped 6.

   This is safe, because `A(3) = 129` is now proven by two independent solvers, but it is
   the tightest quantity in the whole analysis, and it is *derived* rather than chosen.
   **It is now stated in SPEC.md's margin rationale too**, which is where a reader of the
   claim will meet it.  This paragraph used to add that nothing else said so.
   ...
   **DECIDED 2026-08-08: `R*` stays 6/6/6/8 and will not be raised.**  A standing intent
   ...
   rather than only at `C` = 8, was resolved against once the margin question above was
   ...
   that capacity**, which makes it a format decision, not only a margin one.
   `tests/KAT.txt`, the pinned KATs in `tests/tests.cpp`,
   `hash-programs/test-correctness.bash`'s hardcoded digests, SPEC.md's claimed-instances
   table, and the `--help` texts all move with it.

   **What would reopen it is evidence, not preference.**  That evidence is the revision
   trigger published in SPEC.md and CHALLENGES.md, under which a distinguisher reaching 4
   rounds obliges `floor + 4` = 7/7/7/9.  At that point `C` = 6 deserves the first look,
   for the floor reason in the paragraph above, not `C` = 8.
   ...
   **One property of the trigger is worth stating rather than discovering later: it bounds
   ...
   `P` is not a random permutation.  That is deliberate conservatism, since the anchor is
   meant to track what is *structurally* known about `P`.  A future revision that wants to
   qualify it by data complexity should do so explicitly and say why, not by quietly
   reading the trigger more narrowly.

The deliverable is a "Claimed instances" table in SPEC.md, giving `(C, R*, digest sizes)`
per SHA-3 equivalence row, with the margin rationale and a statement that other
parameterizations are supported by the code but **not covered by the claim**.

---

## 5. The evidence program

This section lists what exists, what is missing, and what to add, ordered roughly by value.
...
the full result tables, and
[research/RE-DERIVATION-RUNBOOK.md](research/RE-DERIVATION-RUNBOOK.md) is the procedure for
re-deriving the figures.

**What is still open is recorded in VERIFYING-CLAIMS.md § 16, not here**, and one whole
line of it is closed by decision rather than by result.  Pushing the trail-search
*ceilings* any further (more seeds, more patterns, deeper shells, or alternate MILP optima
above r = 4) **was closed on 2026-08-08 and is not to be re-proposed.**  The ceilings
bracket every shipped round count already, and every floor under them is a converged
optimum, so what a tighter ceiling would buy is characterization, not margin.
...
2. **Trail tightness (new):** the MILP is a lower bound; find *actual* best
   ...
   **DONE** in `research/permute-trail-search.py`, with VERIFYING-CLAIMS § 13 and the
...
3. **Differential clustering (new):** estimate the number of characteristics sharing an
   ...
   **DONE at r = 1, and that is the only round count where it produced a number.**  The
   ...
   probability from 2^−54 to 2^−51.7, so first-order clustering costs about 2 bits, which
   is immaterial against the per-two-round floor.  Above r = 1 every shell enumeration ran
   out of clock `INCOMPLETE`, so the `DP(differential | pattern)` figures they print bound
   nothing and are recorded only as figures to discard (RE-DERIVATION-RUNBOOK.md § 3).  A
   clustering *estimate* at 2–4 rounds therefore does not exist, and obtaining one is
   inside the closed line above.
4. **Structural distinguishers (new; overlap with ADVERSARIAL-REVIEW-PLAN.md §1a/2a —
   ...
   **DONE, and the "test, don't assert" instruction paid off exactly where it was aimed.**
   ...
   covers symmetry-class escape, the fixed-point screen, the round-constant properties,
   and the slide/affine-self-similarity screen.  `permute-invariant-subspaces.py` replaces
   its sampling with an *exhaustive* search over byte-aligned subspaces at every coset
...
5. **Algebraic properties (new):** degree growth of `P` over rounds (the AES S-box is
   ...
   **DONE, and this is the item that ended up carrying the margin policy.**  The degree
   ...
   is anchored to (§4.3 step 3).  So this item is not merely evidence beside the claim but
   an input to the shipped parameters (VERIFYING-CLAIMS § 11 and § 15).
6. **Duplex-PRNG mode note (new, cheap):** the sponge-PRNG papers require *state
   ...
   **DONE, by the documentation route rather than the code route.**  SPEC.md's non-claims
   ...
   paper.  No `forget()` was added, and the non-claim is the deliverable
   (VERIFYING-CLAIMS § 9).
7. **Reduced-round challenge instances (new):** publish KAT-style collision/preimage
   ...
   **DONE in [CHALLENGES.md](CHALLENGES.md), but at 3, 4, and 5 rounds, not the r = 1, 2,
   3, 4 specified here.**  The round counts were moved up deliberately: r = 1 and r = 2 are
...
8. **Statistical batteries (low weight):** PractRand/Dieharder over duplex XOF output at
   ...
   **DONE.**  PractRand ran over 16 GiB of duplex PRNG stream at `C` = 4, at both 6 and 3
   rounds, with no anomalies (`research/duplex-prng-stream.cpp`, VERIFYING-CLAIMS § 12).
   Dieharder was not run, because PractRand is the stronger of the two at this sample
   size, and a second battery would carry the same (zero) cryptographic weight on passing.
   SPEC.md's Evidence section reports it as the smoke test it is, in those words.

---

## 6. Verification guide (user-facing deliverable)

**SHIPPED as [research/VERIFYING-CLAIMS.md](research/VERIFYING-CLAIMS.md)**, in 16 sections.
It opens with the claim → kind → where-to-verify summary this section sketched, and states
...
not this file, is the one to update when a new result lands.
...
* **No security proof of the primitive**, which is impossible in principle.  The claim is a
  conjecture and is labeled as one everywhere.
* **No self-certification.**  Internal analysis (all of §5) makes the claim *well-posed*,
  not *trusted*.  Trust requires third-party cryptanalysis and time, and the challenge
...
* **No side-channel or implementation-security claims**, which are the adversarial
  review's territory.
...
analyses it lists were built (§5), each with its reproduction commands.  One of them, the
algebraic/integral family, did move the best-known-reach number and did update the margin
rationale, exactly as the step anticipated: `R*` = floor + 3 is anchored to the 3-round
inside-out zero-sum.  What is left is not a next step in this order but the standing
...
| Minimization as a ceiling lever | empirical, 2026-08-02 onward | Refuted rather than decided, and recorded here because it keeps looking attractive: 31 attempts at 600 s produced 0 improvements, and no minimization has ever completed at r ≥ 2. |

**What reopens the first entry, and only it, is the published revision trigger.**  A
