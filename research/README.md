...
A bracket needs a **proven** _A_ for its floor.  The 2026-08-01/02 re-derivation of the MILP table (above) left only _r_ ≤ 2 solved at _N_ = 16, so at _r_ ≥ 3 the floor no longer comes from the solver.  It comes instead from superadditivity, which is structural rather than empirical: any (_a_+_b_)-round trail restricts to an _a_-round trail and a _b_-round trail — each with a nonzero input difference, since `P` is a bijection — over disjoint S-box layers, so **_A_(_a_+_b_) ≥ _A_(_a_) + _A_(_b_)**.  Applied to the two proven values _A_(1) = 9 and _A_(2) = 45 this gives _A_(3) ≥ 54, _A_(4) ≥ 90 and _A_(5) ≥ 99 — weaker than the solved figures the table used to carry, but proven, and enough to keep every round count bracketed except _r_ = 5, where no trail has been found for the ceiling.
...
| 3 | 9 | 54 … ≤ 129 (was ~~133~~) | 2<sup>−324</sup> | 2<sup>−903</sup> | [324, 903] |
| 4 | 12 | 90 … ≤ 165 (was ~~225~~) | 2<sup>−540</sup> | 2<sup>−1154</sup> | [540, 1154] |
| 5 | 15 | 99 … ≤ 243 | 2<sup>−594</sup> | none found | floor only — stage A times out |

The _r_ = 3 and _r_ = 4 brackets are far wider than the ones this table used to report, and deliberately so: their floors are now derived from two proven values rather than solved directly, and their ceilings come from cheaper activity patterns than were previously known.  Every floor is still comfortably past 2<sup>−256</sup>.
...
**Only _r_ ≤ 2 is bracketed by solved values.**  The _r_ = 3 and _r_ = 4 brackets ([324, 903] and [540, 1154]) rest on floors derived by superadditivity from _A_(1) and _A_(2), not on a converged solve at those round counts, and their ceilings are single found trails rather than minima — so both ends are loose and the interval is correspondingly wide.  At _r_ = 5 there is a floor (2<sup>−594</sup>) but no ceiling, since stage A never produces a pattern to instantiate.  _r_ ≥ 6 is unsearched.  Read these as bounds, not as the permutation's actual trail weights.  See [VERIFYING-CLAIMS.md](VERIFYING-CLAIMS.md) for how these results feed the claim.
