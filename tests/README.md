...
[KAT.txt](KAT.txt) is a machine-readable known-answer-test file pinning the digests of `Castella::Duplex`, `Castella::DuplexTree`, the keyed (MAC) construction, and `compress_castella_tree` across parameter and message-length sweeps (chunk, rate, and compression-block boundaries, and the leaf-index and MAC-key 255/256 `left_encode` byte-width boundaries).  Each line is self-describing.  The message is the deterministic pattern `msg[i] = i mod 256` and a MAC key is `key[i] = 255 - (i mod 256)`, so external implementations can consume the file directly.  A `mac` line is not a separate class.  It is a `Castella::DuplexTree` over `bytepad(encode_string(K), chunk) || msg || right_encode(out)`, the construction [SPEC.md](../SPEC.md) specifies for `castella --key-file`.

[research/spec-conformance.py](../research/spec-conformance.py), an independent pure-Python implementation written from [SPEC.md](../SPEC.md) alone, verifies every line, so the spec, this file, and the C++ implementation must be kept in agreement whenever the digests change.

* `./kat` verifies every line against the current implementation, with a nonzero exit status on any mismatch.
* `./kat --generate > KAT.txt` regenerates the file, which is only for when the digests deliberately change.  That same change voids the published targets in [CHALLENGES.md](../CHALLENGES.md), which are digests of specific instances rather than of this file's sweeps, so regenerate them in the same pass.
* `./kat --generate | diff - KAT.txt` checks that the committed file is still exactly what the generator emits.  `make test` runs this after `./kat`, and it covers what `./kat` structurally cannot.  Verification recomputes each digest from the line's own fields, so editing a swept *value* in `generate()` leaves the file testing the old value and every check still passes.  (`EXPECTED_KATS` already catches a change in the *number* of vectors.)  A failure here is not a digest mismatch, and the fix is to regenerate the file or revert the sweep.

### `equivalence-tests`

The digest of a tree hash is a function of the node parameters, tree geometry, and input bytes only, never of the `add()` granularity, thread count, or parallel path.  `equivalence-tests` hammers that contract with randomized inputs at adversarial sizes (chunk boundaries, the streaming-pool start threshold, the paired-leaf index-width fallback, and random lengths).  For each size, the single-threaded one-shot digest is the reference, and every combination of {one-shot, randomly split adds} × {thread counts} must reproduce it.  The seed is printed, and can be passed as an argument, so failures reproduce.

### `permute-equivalence`

`Castella::permute` dispatches to `Castella::permute_folded` (the folded, register-resident implementation, x86-64 with VAES only) or to `Castella::permute_generic` (everywhere else).  The two must be bit-identical, or every digest here would depend on which path the build selected.  The KATs guard that only transitively, since a folded build reproduces the KATs the pure-Python model produces, but no build compares the two paths directly.  This program does, over random states, for every supported state size (2, 4, 8, and 16 blocks) and every round count from 0 to `NUM_ROUNDS_MAX`.  The seed is printed, and can be passed as an argument, so failures reproduce.
...
Every `duplex` line in KAT.txt has the same shape (construct, one `add`, one `squeeze`), so the KAT file pins the algorithm but leaves most of the `Castella::Duplex` API untouched.  This fuzzer covers the rest.  It generates random programs of `add` / `add_left_encoded` / `add_right_encoded` / `apply_padding_rule` / `squeeze_bytes` calls over random constructor parameters.  It replays each one against the `Duplex` in [research/spec-conformance.py](../research/spec-conformance.py), imported directly so there is no second copy of the model to drift, and diffs every squeeze against `duplex-diff-driver`, which interprets the same script.  Split and streamed adds, both encoding entry points (no KAT drives either), explicit padding, and successive squeezes are all reachable only this way.
...
The fuzzer avoids, rather than models, one `Castella::Duplex` convenience the specification does not describe, because it is C++ API behavior rather than digest behavior.  `squeeze_bytes(n)` clamps *n* where the model raises on an out-of-range value.
