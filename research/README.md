...
* For _N_ = 16 with _a_ = 3, two rounds already bound every characteristic below 2<sup>−270</sup>, past the 2<sup>−256</sup> threshold.  The former <q>three rounds give 2<sup>−798</sup></q> is withdrawn, since _A_ = 133 was refuted — but a comparable bound survives from a different source.  **CBC's dual bound is itself a proven lower bound**, valid at any point in the search and not only when the gap closes: a 90-minute run at _r_ = 3 ends holding `Lower bound: 127.554`, so _A_(3) ≥ **128** and DP ≤ 2<sup>−768</sup>.  With the incumbent at 129 the true value is one of two integers, {128, 129}.
* Above _r_ = 3 the floors come from superadditivity, which is structural rather than empirical: _A_(_a_+_b_) ≥ _A_(_a_) + _A_(_b_) holds because `P` is a bijection, so a longer trail restricts to two shorter ones with nonzero input differences over disjoint S-box layers.  From _A_(1) = 9, _A_(2) = 45 and _A_(3) ≥ 128 this gives _A_(4) ≥ **137** (DP ≤ 2<sup>−822</sup>) and _A_(5) ≥ **173** (DP ≤ 2<sup>−1038</sup>).
* **Neither source dominates, so take the maximum of the two.**  At _r_ = 3 the solver's dual bound (128) is 2.4× the superadditive 54; at _r_ = 4 it is the other way round, a 90-minute run reaching only 93.9 against the superadditive 137.  The crossover has a reason: superadditivity strengthens as _r_ grows, because it composes an increasingly good _A_(3) with _A_(1), while the MILP weakens, because each extra round adds layers to a relaxation that is already struggling — the final duality gap is 1% at _r_ = 3 and 76% at _r_ = 4.  Note that `permute-min-active-sboxes.py` reports only what its own instance proves, so at _r_ = 4 it prints `A in [94, 165]` while the floor recorded here is the larger 137.
...
A bracket needs a **proven** _A_ for its floor.  The 2026-08-01/02 re-derivation of the MILP table (above) left no round count above 2 with a closed duality gap at _N_ = 16, so the floors at _r_ ≥ 3 come from two other sources, both proven.  At _r_ = 3 it is **CBC's dual bound**, which is a lower bound on the optimum at every point in the search and not only when the gap closes: a 90-minute run ends at 127.554, so _A_(3) ≥ 128.  Above that it is **superadditivity**, which is structural rather than empirical: any (_a_+_b_)-round trail restricts to an _a_-round trail and a _b_-round trail — each with a nonzero input difference, since `P` is a bijection — over disjoint S-box layers, so **_A_(_a_+_b_) ≥ _A_(_a_) + _A_(_b_)**.  From _A_(1) = 9, _A_(2) = 45 and _A_(3) ≥ 128 this gives _A_(4) ≥ 137 and _A_(5) ≥ 173.
...
| 3 | 9 | 128 … ≤ 129 (dual bound; was ~~133~~) | 2<sup>−768</sup> | 2<sup>−903</sup> | [768, 903] |
| 4 | 12 | 137 … ≤ 165 (was ~~225~~) | 2<sup>−822</sup> | 2<sup>−1154</sup> | [822, 1154] |
| 5 | 15 | 173 … ≤ 243 | 2<sup>−1038</sup> | none found | floor only — stage A times out |

The _r_ = 3 floor is within one active S-box of its ceiling: _A_(3) is either 128 or 129, and only the last unit of duality gap is missing.  Every floor is far past 2<sup>−256</sup>.
...
* **_r_ = 3: both ends moved, to [768, 903].**  This round count used to report [798, 928] on a 133-box pattern.  The MILP re-derivation refuted _A_ = 133 (a feasible 129-box pattern exists), so 6·_A_ = 798 was never a lower bound; the floor comes instead from CBC's dual bound of 127.554 after 90 minutes, giving _A_(3) ≥ 128 and a weight floor of 768.  That is only 30 bits below the withdrawn 798, so the bracket is barely wider at the bottom than the one it replaces — and unlike its predecessor it is sound.  The ceiling moved too, and in the same direction: asking the trail search for the smaller pattern directly — `-A 129` — produced a realizable trail of weight **903** in 7.6 s of solver time, lighter than the 928 that stood before.  Both are the same finding seen twice — a cheaper activity pattern exists, and it carries a cheaper characteristic.  For the record, the historical figures on the 133-box pattern remain valid as characteristics (`witness` reached 928 in ~25 min, `rows` reached 929 in 3.8 s); they are simply trails through a non-minimal pattern, so 903 supersedes them as the ceiling.
* **_r_ = 4: the old bracket never contained the answer.**  This round count used to report [1350, 1573] on the 225-box pattern.  The MILP re-derivation refuted _A_ = 225 (a feasible 165-box pattern exists), and running the trail search at `-A 165` produced a realizable characteristic of weight **1154** in 4.5 s of solver time — **below the old bracket's lower endpoint of 1350**.  That is the sharpest available statement about the old figure: it was not a conservative floor since improved upon, it was an interval the true value was never inside.  The corrected bracket is **[822, 1154]**, its floor from the superadditive _A_(4) ≥ _A_(1) + _A_(3) ≥ 9 + 128 = 137.  The weight-**1573** trail on the 225-box pattern is still a genuine characteristic (three independent runs returned it, at 5.1 s, 5.9 s and 5.9 s of solver time, 890 MB peak) — it is simply a trail through a non-minimal pattern, so it bounds nothing that 1154 does not bound better.  The encoding comparison this paragraph recorded is unaffected and still stands: on that pattern `witness` (`--patterns 1 --no-minimize -t 1800`) found **no trail at all**, giving up at the time limit while peaking at 6.38 GiB, whereas `rows` finished in under 3 min wall — the widest the gap between the encodings has been measured, and the only round count where it is unbounded rather than a ratio.

* **_r_ = 5: nothing at all, and the wall is in a different place.**  At _r_ ≤ 4 stage A returns an activity pattern in well under a second and any difficulty is stage B's.  At _A_ = 243, stage A itself gave up (`unknown: timeout`) — after 30 min, and again on a rerun with `-t 3300`, so no pattern has ever reached the bit level and both runs ended with nothing to instantiate.  The 2<sup>−1458</sup> this paragraph used to quote as the surviving floor is not one: 243 is an unconfirmed incumbent, not a proven optimum.  The floor that does survive is the superadditive _A_(5) ≥ _A_(2) + _A_(3) ≥ 45 + 128 = 173, i.e. 2<sup>−1038</sup>.  So this round count has a floor and no ceiling — the mirror image of the situation two rounds down.  The failure is worth distinguishing from the old _r_ = 4 `witness` failure: there the pattern existed and could not be realized in the time given; here the search never got a pattern to try.  The likely reason is that the target grows only 225 → 243 while the layer count grows 12 → 15, so an exact-cardinality constraint (`PbEq` = 243) has to place activity more thinly across a larger structure — tight cardinality over more variables, which is the hard regime for this kind of SAT encoding.  A larger `-t` aimed at stage B cannot help.  Unlike before, `-A` now _might_: 243 is a guess rather than a proven optimum, and the neighbouring round counts show that asking for the right smaller target (`-A 129` at _r_ = 3, `-A 165` at _r_ = 4) turns an intractable search into a few seconds of solving.  Peak memory was 323 MB, but only because the run never reached stage B — it is not comparable with the _r_ ≤ 4 figures.
...
**Only _r_ ≤ 2 is bracketed by solved values.**  The _r_ = 3 bracket ([768, 903]) rests on a dual bound from a run that stopped 1.45 units short of closing, and the _r_ = 4 one ([822, 1154]) on superadditivity applied to it; both ceilings are single found trails rather than minima, so neither end of either interval is tight.  At _r_ = 5 there is a floor (2<sup>−1038</sup>) but no ceiling, since stage A never produces a pattern to instantiate.  _r_ ≥ 6 is unsearched.  Read these as bounds, not as the permutation's actual trail weights.  See [VERIFYING-CLAIMS.md](VERIFYING-CLAIMS.md) for how these results feed the claim.
...
# r = 4: weight 1154, ~5 s of solving.  Same caveat (A=165 is an incumbent).
...
* `-t` is the per-solver-call time limit.  No minimization has ever finished at _r_ ≥ 2 on this machine; the reported 302, 903 and 1154 are the best trails found, not proven minima, and a longer limit may or may not tighten them.  Expect run-to-run variation in which trail is found (302, 312, 313, 314 and 315 have all come back at _r_ = 2).
...
An outbound spanning `r_out` rounds split as `r_top + r_bot` has `A_out ≥ A(r_top) + A(r_bot)`, and the attacker picks the split minimizing that sum.  The inputs are the two solved values `A(1)=9` and `A(2)=45`, CBC's dual bound `A(3) ≥ 128`, and `A(4) ≥ 137` from superadditivity (`A(a+b) ≥ A(a)+A(b)`, valid because `P` is a bijection).  Minimizing over integer splits gives the attacker-optimal outbound cost `2^(6·A_out)`:
...
For `C` = 8 (claim 2^512, run at `R*` = 8 rounds) a 3-round inbound leaves `r_out` = 5 and an outbound of ≥ 2^876, safe by 2^364 — down from the 2^1068 previously claimed, since `A(4) ≥ 137` is derived rather than solved.  A 5-round inbound would leave `r_out` = 3 at 2^324, **below** that instance's 2^512 claim; the earlier text quoted the same 2^324 in a way that read as reassuring, which it is not at `C` = 8.
