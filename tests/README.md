...
[KAT.txt](KAT.txt) is a machine-readable known-answer-test file pinning the digests of `Castella::Duplex`, `Castella::DuplexTree`, the keyed (MAC) construction, and `compress_castella_tree` across parameter and message-length sweeps (chunk/rate/compression-block boundaries, the leaf-index and MAC-key 255/256 `left_encode` byte-width boundaries).  Each line is self-describing; the message is the deterministic pattern `msg[i] = i mod 256` and a MAC key is `key[i] = 255 - (i mod 256)`, so external implementations can consume the file directly.  A `mac` line is not a separate class: it is a `Castella::DuplexTree` over `bytepad(encode_string(K), chunk) || msg || right_encode(out)`, the construction [SPEC.md](../SPEC.md) specifies for `castella --key-file`.  [research/spec-conformance.py](../research/spec-conformance.py) — an independent pure-Python implementation written from [SPEC.md](../SPEC.md) alone — verifies every line, so the spec, this file, and the C++ implementation must be kept in agreement whenever the digests change.

* `./kat` verifies every line against the current implementation (nonzero exit status on any mismatch).
* `./kat --generate > KAT.txt` regenerates the file — only to be done when the digests deliberately change.  That same change voids the published targets in [CHALLENGES.md](../CHALLENGES.md), which are digests of specific instances rather than of this file's sweeps; regenerate them in the same pass.
...
One `Castella::Duplex` convenience that the specification does not describe is deliberately avoided rather than modeled, since it is C++ API behavior rather than digest behavior: `squeeze_bytes(n)` clamps *n* where the model asserts the range.
