...
An instance `(C, num_rounds)` is claimed iff `num_rounds ≥ R*` for its row.  **Margin rationale:** `R*` = 2 × max(diffusion floor, trail floor).  The diffusion floor is 3 rounds (full bit diffusion; see Evidence).  The trail floor is the smallest round count whose proven per-characteristic bound `2^−6·A` is at most `2^−2b` for claimed level `b` — twice the exponent of the generic bound, as headroom for the trail clustering the model does not cover.  From the Evidence table, 2 rounds cover `b` ≤ 135, 3 rounds `b` ≤ 387, 4 rounds `b` ≤ 414 and 5 rounds `b` ≤ 522.  Hence `R*` = 2·max(3, 2) = 6 for `C` = 2 and 2·max(3, 3) = 6 for `C` ∈ {4, 6}.  The factor 2 is a deliberate margin against the structural attacks the bounds do not address and will be revisited as evidence accumulates.

**`C` = 8 is a documented exception to that rationale, not an application of it.**  Its trail floor is 5 rounds, so the same formula would give `R*` = 10 rather than the 8 published above; the shipped value is 1.6× its floor where every other row is 2× or better.  This is a change of evidence, not of design: earlier revisions put the 4-round bound at `2^−1350` (`A` = 225), which made 4 the floor and 8 exactly 2×.  That figure was a timed-out solver incumbent mislabelled as an optimum, and it has been refuted — a 165-box activity pattern is known to exist at 4 rounds, so `6·A(4) ≤ 990 < 1024` holds regardless of what the solver eventually proves, and 4 rounds cannot reach the 512-bit level under this criterion however the remaining gap closes.  The `C` = 8 row is therefore claimed on a thinner margin than the others until either the policy or the round count is revisited; the other three rows are unaffected, since their floors rest only on the 1- and 2-round bounds, which are solved.
...
* A byte-level truncated-differential MILP model gives **proven lower bounds** on differentially active AES S-boxes per characteristic.  The solver converges only through 2 rounds of `P`, where it proves **45 active S-boxes**, hence DP ≤ 2^−270 — past the 2^−256 threshold.  At 3 rounds it proves **A(3) = 129** (DP ≤ 2^−774), which required exploiting the objective's integrality — the optimum is a whole number, so the search can stop once the dual bound exceeds incumbent − 1 rather than closing the gap entirely.  Beyond that the bounds come from superadditivity: any (a+b)-round trail restricts to disjoint a- and b-round trails with nonzero input differences (`P` is a bijection), so A(a+b) ≥ A(a) + A(b), giving A(4) ≥ **138** (DP ≤ 2^−828), A(5) ≥ **174** (DP ≤ 2^−1044) and A(6) ≥ **258** (DP ≤ 2^−1548).  Superadditivity is now the only source above 3 rounds that contributes anything: a 55-minute solve at 6 rounds returns a dual bound of 13 against its own incumbent of 290, far weaker than the 258 the composition gives for free.  The same counts bound linear-trail correlations by 2^−3·A.  Figures of 133 for 3 rounds (DP ≤ 2^−798) and 225 for 4 (DP ≤ 2^−1350) appeared in earlier revisions of this document as proven lower bounds; they were timed-out solver incumbents mislabelled as optima, and both have since been refuted by cheaper activity patterns.  These bounds cover **single characteristics only** — nothing about differential clustering, rebound attacks, invariant subspaces, or other structural distinguishers — so they are necessary, not sufficient.
* A bit-level SAT/SMT search for *actual* characteristics (`research/permute-trail-search.py`) checks the model from the other side.  For 1 round the bound is **proven tight**: a real characteristic attains all 9 active S-boxes at the maximum 2^−6, weight exactly 54 = 6·A.  For 2 rounds a characteristic is realizable near the floor (best found weight 302 vs. the 6·A = 270 floor), but exact minimization is intractable, so the best-trail weight is only bracketed in [270, 302] — under 1 bit per S-box above the floor.  3 and 4 rounds are bracketed more loosely, [774, 891] and [828, 1153] — 3 rounds has a solved floor but 4 rounds only a superadditive one, and both ceilings are the best of several found trails (each re-verified transition-by-transition against the AES DDT), not minima.  At 5 rounds the search finds no characteristic at all for the shipped 16-block state, at any activity target tried, so that round count has a floor and no ceiling.  Enumerating the *full* differential for the 1-round optimum (1048 characteristics) raises its probability from 2^−54 to 2^−51.7: first-order clustering costs about 2 bits here, immaterial against the per-two-round floor.  Real trails never fall below the proven floors, so the bounds stay conservative.
