...
- **`compress_castella_hash_x2`** (`include/cch-x2.hpp`) instead owns two ordinary nodes (friend of cch) and interleaves their compression chains in one bulk loop — no lane packing. A single cch node is latency-bound across chunks, measured ~1.06× at the default chunk size and 1.04–1.13× across the cache-resident sizes by `research/simd_compress-two-state-benchmark.cpp` (a wash at `CHUNK_SIZE_MIN`), verified by `research/cch_x2-verify.cpp`.
