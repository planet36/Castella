...
* **Differential trails.**  The MILP lower bound is A = 45/133/225 active S-boxes at r = 2/3/4 (§4).  Real-trail tightness is settled only at r = 1 (weight 54, proven optimal for its pattern); r = 2 is bracketed **[270, 302]**, r = 3 is bracketed **[798, 928]**, and r ≥ 4 has no bracket at all — a 30-minute `witness` probe at r = 4 found no bit-level trail, so only the MILP floor stands there (§13, §16).  Both brackets have a proven floor and a merely-found ceiling, so a verified trail below either floor, a lighter trail tightening either ceiling, or a proof closing either gap is a direct result — as is a first realizable characteristic at r = 4, which would establish that bracket rather than tighten one.
