...
| 3 | 9 | **129** (solved; was ~~133~~) | 2<sup>−774</sup> | 2<sup>−846</sup> | [774, 846] |
...
Every round count with both ends now has a **solved** floor, so the remaining width is entirely the trail search's: 161 bits at _r_ = 4, 72 at _r_ = 3, 24 at _r_ = 2.  The _r_ = 4 figure used to read 325 against a superadditive floor of 828 — solving _A_(4) = 165 raised that floor to 990 and halved the apparent gap, which is what a loose floor costs when it is mistaken for the trail's slack.  Every floor is far past 2<sup>−256</sup>.
...
# The recorded ceilings (846 and 1151) come from sweeping BOTH the pattern count and
# the seed: run each K in 0..7 as its own process, in parallel, and take the lightest.
...
# (~100 s per pattern at r = 3 against a printed ~8 s).  ~35 min wall at r = 3 for
# eight parallel seeds, ~40 at r = 4; under 2 GiB total with -M.  Seed 0 pattern 12
# gives the 846; five of the r = 4 trails tie at 1151.
for K in 0 1 2 3 4 5 6 7; do
    python3 permute-trail-search.py -r 3 -A 129 --patterns 16 --random-seed "$K" \
        --no-minimize -t 600 -M 900 > "r3-seed$K.txt" &
done; wait
grep -h 'best characteristic' r3-seed*.txt   # then the same with -r 4 -A 165 -M 1200
