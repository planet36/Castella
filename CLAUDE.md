...
- **`examples/`** — Usage demonstrations that are also a real test suite: hash (cSHAKE-like), MAC (KMAC/KMACXOF-like), TupleHash(XOF)-like, and ParallelHash(XOF)-like usage, checked against hardcoded expected outputs.
  ...
  - *Preamble*: **do not remove the `#define DEBUG 1` / `#undef NDEBUG` preamble at the top of `examples.cpp`.**  It is not there for this file's own checks, which do not use `assert`.  It arms the library's internal assertions, which `include/*.hpp` gate on `#if defined(DEBUG)`.  This translation unit has dozens of assertion sites with the preamble, and none at all without it, since a release build defines no `DEBUG`.  The same preamble appears in several of the `research/*.cpp` for the same reason (`grep -l '#define DEBUG 1' research/*.cpp`).
