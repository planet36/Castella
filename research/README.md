...
| permute\_model.py | The shared model of `P`'s layers (AES S-box, DDT, ShiftRows, MixColumns, one aesenc round, and the simd\_transpose map), imported by every program below that models the permutation.  Pure standard library, and it self-tests on import, so every importer gets validated layers.  `spec-conformance.py` does **not** import it, because it is the independent implementation that `trail-model-crossvalidate.py` checks this model against |
...
* **x86-64-with-VAES only.**  These measure code paths that exist only there, and elsewhere compile to a stub that prints `skipped`: `permute_folded-benchmark`, `permute_x2-benchmark`, `aes_enc_arr-benchmark`, `aes_enc_arr_cast-benchmark`, `nested-for-loop-order-aes_enc_0-benchmark`, and `simd_compress_aes_enc-num_rounds-benchmark`.
