...
* A bit-level SAT/SMT search for *actual* characteristics realizing those MILP bounds (`research/permute-trail-search.py`) checks the bounds from the other side.  For 1 round the bound is **proven tight**: a real characteristic attains all 9 active S-boxes at the maximum 2^−6, weight exactly 54 = 6·A.  For 2 rounds a characteristic is realizable near the floor (weight ≤ 315 vs. the 6·A = 270 floor), but exact minimization is intractable, so the best-trail weight is only bracketed in [270, 315] — at most 1 bit per S-box above the floor.  Enumerating the *full* differential for the 1-round optimum (847 characteristics) raises its probability from 2^−54 to 2^−51.8: first-order clustering costs about 2 bits here, immaterial against the per-two-round floor.  Real trails never fall below the MILP floor, so the bounds stay conservative.
