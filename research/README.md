...
* **Where a solve is out of reach, the two remaining sources do not dominate each other, so take the maximum.**  At _r_ = 4 a 90-minute run reaches a dual bound of only 93.9, a 76% gap, and is beaten by the superadditive 138.  The reason is structural: superadditivity strengthens as _r_ grows, because it composes an increasingly good _A_(3) with _A_(1), while the MILP weakens, because each extra round adds layers to a relaxation that is already struggling.  Note that `permute-min-active-sboxes.py` reports only what its own instance proves, so at _r_ = 4 it prints `A in [94, 165]` while the floor recorded here is the larger 138.
...
| 3 | 9 | **129** (solved; was ~~133~~) | 2<sup>−774</sup> | 2<sup>−903</sup> | [774, 903] |
| 4 | 12 | 138 … ≤ 165 (was ~~225~~) | 2<sup>−828</sup> | 2<sup>−1154</sup> | [828, 1154] |
| 5 | 15 | 174 … ≤ 243 | 2<sup>−1044</sup> | none found | floor only — stage A times out |

Only _r_ = 4 now separates a solved floor from a found ceiling by a wide margin: 326 bits, against 129 at _r_ = 3 and 32 at _r_ = 2.  Every floor is far past 2<sup>−256</sup>.
...
**_r_ ≤ 3 is bracketed by solved values; _r_ = 4 is not.**  The _r_ = 3 bracket ([774, 903]) has a proven floor, and the _r_ = 4 one ([828, 1154]) rests on superadditivity applied to it; every ceiling is a single found trail rather than a minimum, so no upper end is tight.  At _r_ = 5 there is a floor (2<sup>−1044</sup>) but no ceiling, since stage A never produces a pattern to instantiate.  _r_ ≥ 6 is unsearched.  Read these as bounds, not as the permutation's actual trail weights.  See [VERIFYING-CLAIMS.md](VERIFYING-CLAIMS.md) for how these results feed the claim.
