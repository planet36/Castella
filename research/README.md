...
| permute-division-property.py | Bit-based division property of `Castella::permute`: decides whether a *chosen* cube leaves each output bit balanced, so it reaches the structured (byte-aligned) cubes the random zero-sum probes cannot draw; validated by reproducing AES's Square distinguisher in both directions |
...
* Scope: black-box **random** cubes only.  Structured cube choices (e.g. positioned to exploit AES's column structure), higher dimensions (degree after 2 rounds is bounded by ~49 per AES-layer counting, out of reach of a 2^49 cube), and inside-out zero-sums that run `P` and `P⁻¹` from a middle state are not covered — those are the standard next steps of an algebraic analysis, not a gap in this probe's claim.  **The first two of those three are now covered**, by `permute-division-property.py` in the next section, which decides a chosen cube instead of sampling; inside-out zero-sums remain uncovered.  Even the 1-round single-block row moves: a *byte-aligned* cube of the same dimension 8 zeroes all 2048 bits, not 1920.
...
* **There is a 2-round integral distinguisher, with 2^128 data.**  It has an exact structural proof, independent of the solver, and the proof is the more useful object:
  ...
  3. Round 2 within each block is then exactly the AES 3-round Square setup, which balances all 128 of its output bits.  The closing transpose only permutes bytes.

  The mechanism is the transpose turning **one bijective block into sixteen active bytes** — precisely the structure the Square attack consumes.  This is the sharpest statement the transpose's block/byte exchange has yet produced, and unlike the invariant-subspace case the round constants are irrelevant to it: an integral property is unaffected by constant addition.

  Those three steps are a complete proof on their own — steps 1 and 2 are exact, step 3 is the Square distinguisher that `--validate` reproduces, and the closing transpose only permutes — so the result does not rest on the solver.  The model agrees with it anyway, across the whole state: a full `--count` scan returns **all 16 target blocks balanced, 2048 of 2048 bits**, with no timeouts and no unknowns.  Budget **~50 min**.
...
* **No margin moves, and it is worth being precise about why.**  The algebraic-degree bound already concedes that a degree-based zero-sum / integral distinguisher reaches **at most ≈ 2.67** of the 6 default rounds.  This distinguisher sits at **2** rounds, inside that concession.  What changes is the *kind* of statement available: the reach was bounded only from above, and is now realized from below by an explicit construction, so it is **bracketed at [2, 2.67] rounds** instead of merely capped.  Supplying that matching lower end is exactly what a division-property model was wanted for.  `R*` is 6 (8 at `C` = 8), the flat sponge claim already concedes `P` is not a random permutation, and the rebound margin argument rests on active-S-box counts that this does not touch.

Scope, and where the model stops.  The technique decides the cube it is given; it says nothing about cubes it is not given, and a SAT is never evidence of absence.  Inside-out zero-sums — running `P` and `P⁻¹` from a middle state — are still not covered, and remain the one item of the section above's three that is open.  At _r_ = 3 the sparse construction still builds (18 live blocks, 864 S-boxes, 55,616 variables, ~130–165 s).  **Its first check now resolves, given enough time, and the answer is SAT**: with the single-block cube, output bit 0 of block 0 is *not* provably balanced, so there is **no full 2048-bit zero-sum at 3 rounds by this model for that cube**.  The earlier report that "the checks do not resolve" was a budgeting artifact — the query needed **~6 200 s of solve**, against the 600 s it had been given.  Read the direction carefully, because this is the weak half of the dichotomy: a SAT bounds the *technique*, never the permutation, so 3 rounds is **not refuted** — what is now excluded is only the full-state zero-sum for this cube under this model.  Two things stay open at _r_ = 3: partial balance on other output bits (the scan stops at the first SAT unless `--count` is passed, so nothing is known about bits 1 onward), and the larger `two-blocks` cube, which is an *easier* balancedness target and has not been tried.  The same asymmetry as the trail search shows up throughout: finding a trail (SAT) is cheap and proving none exists (UNSAT) is what costs, and it is the UNSAT side that carries every positive result above — the 3-round SAT taking 6 200 s is the exception that shows how little that asymmetry guarantees at this width.
...
* This is an **upper** bound on the degree, so its direct use is the attacker's: where the bound is < `n − 1`, a distinguisher provably exists; where it equals `n − 1`, the method is simply silent.  It therefore bounds the reach of the degree-based construction — it does **not** prove that no integral distinguisher exists beyond 2.67 rounds.  A matching lower bound needs a bit-based division-property model, and **that model now exists** — `permute-division-property.py` realizes an explicit 2-round integral distinguisher, so the reach is bracketed **[2, 2.67]** rounds rather than only capped here.  What is still open is the gap itself: 3 rounds is undecided for the permutation, since the model's only resolved check there is a SAT, which bounds the technique and not `P`.
