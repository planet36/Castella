...
**_r_ ≤ 3 is bracketed by solved values; _r_ = 4 is not.**  The _r_ = 3 bracket ([774, 891]) has a proven floor, and the _r_ = 4 one ([828, 1153]) rests on superadditivity applied to it; every ceiling is a found trail rather than a minimum — the best of eight patterns at _r_ = 3 and of three at _r_ = 4 — so no upper end is tight.  At _r_ = 5 there is a floor (2<sup>−1044</sup>) but no ceiling at _N_ = 16, since stage A never produces a pattern to instantiate, at any of the six targets tried.  The _r_ = 5 ceilings that do exist — [606, 705] at _N_ = 2 and [684, 791] at _N_ = 4 — bound **narrower permutations, not the shipped one**, and must not be read as bounds on _N_ = 16.  _r_ ≥ 6 is unsearched by the trail search.  Read these as bounds, not as the permutation's actual trail weights.  See [VERIFYING-CLAIMS.md](VERIFYING-CLAIMS.md) for how these results feed the claim.
