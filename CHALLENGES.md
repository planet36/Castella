...
* **Differential trails.**  The MILP lower bound is A = 45/133/225/243 active S-boxes at r = 2/3/4/5 (§4).  Real-trail tightness is settled only at r = 1 (weight 54, proven optimal for its pattern); r = 2 is bracketed **[270, 302]**, r = 3 **[798, 928]**, r = 4 **[1350, 1573]**, and r ≥ 5 has no bracket at all — only the MILP floor stands there (§13, §16).  Every bracket has a proven floor and a merely-found ceiling, so a verified trail below any floor, a lighter trail tightening any ceiling, or a proof closing any gap is a direct result — as is a first realizable characteristic at r ≥ 5, which would establish a bracket rather than tighten one.  At r = 5 the obstacle is worth knowing before starting: the published search stalls in *stage A*, unable to produce a minimal activity pattern within 30 min, so the bit-level instantiation is not the part that needs beating — a better pattern search, or a pattern obtained by other means, is.
